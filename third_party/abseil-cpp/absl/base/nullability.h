...
// absl:google3-begin(internal comment)
//
// However, we do provide some verification of annotated code: the ClangTidy
// check go/clang-tidy/checks/google3-runtime-pointer-nullability is enabled for
// google3 (except third_party) and runs in Tricorder (for snapshots and
// presubmits). Thus, the check *encourages* adherence to the annotations, but
// does not *enforce* it -- violations will neither break builds nor block
// submit.
//
// In addition, compilers that support `nullability_on_classes` do some limited
// checking of nullability annotations at compile time; for example, checking
// that nullability annotations in the declaration and definition of a function
// match.
// absl:google3-end
// ===========================================================================
...
#include "third_party/abseil-cpp/absl/base/internal/nullability_impl.h"
...
//
// absl:google3-begin(only understood by google3-internal clang-tidy check)
#if defined(__clang__)
// clang-format off
#define ABSL_POINTERS_DEFAULT_NONNULL                       \
  _Pragma("clang diagnostic push")                          \
  _Pragma("clang diagnostic ignored \"-Wunknown-pragmas\"") \
  _Pragma("nullability file_default nonnull")               \
...
#endif  // THIRD_PARTY_ABSL_BASE_NULLABILITY_H_